  ...
  return (
    <div>
      <h2 id="food-heading" data-cy="FoodHeading">
        <Translate contentKey="smartServeApp.food.home.title">Foods</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="smartServeApp.food.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/food/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="smartServeApp.food.home.createLabel">Create new Food</Translate>
          </Link>
        </div>
      </h2>
        ...
        {foodList && foodList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="smartServeApp.food.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                <th className="hand" onClick={sort('name')}>
                  <Translate contentKey="smartServeApp.food.name">Name</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('name')} />
                </th>
            ...
            <tbody>
              {foodList.map((food, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{food.available ? 'true' : 'false'}</td>
                  <td>{food.imageUrl}</td>
                  <td>{food.createdDate ? <TextFormat type="date" value={food.createdDate} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>{food.updatedDate ? <TextFormat type="date" value={food.updatedDate} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>{food.deletedDate ? <TextFormat type="date" value={food.deletedDate} format={APP_DATE_FORMAT} /> : null}</td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        tag={Link}
                        to={`/food/${food.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      ...
                      <Button
                        onClick={() =>
                          (window.location.href = `/food/${food.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`)
                        }
                        color="danger"
                        size="sm"
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="smartServeApp.food.home.notFound">No Foods found</Translate>
            </div>
          )
        )}
      </div>
      {totalItems ? (
        <div className={foodList && foodList.length > 0 ? '' : 'd-none'}>
